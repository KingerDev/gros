...
<template>
    ...
    <GrosLayout title="Analýzy" subtitle="Kam tečú peniaze a ako sa to vyvíja">
        <div class="gros-rise">
            ...
            <!-- Mesiac v kocke -->
            <div v-if="monthReport" style="margin-bottom: 14px">
                <Card :title="`Mesiac v kocke · ${monthReport.label}`">
                    <template #right>
                        <span style="font-size: 12px; font-weight: 600; color: #9a9cab"
                            >{{ monthReport.count }} transakcií · porovnané s {{ monthReport.prevLabel }}</span
                        >
                    </template>
                    <div style="display: flex; flex-wrap: wrap; gap: 22px; margin-top: 16px">
                        <div style="flex: 1; min-width: 130px">
                            ...
                            >
                                {{ eur(monthReport.income) }}
                            </div>
                            <DeltaBadge :pct="monthReport.incomeDeltaPct" />
                        </div>
                        <div style="flex: 1; min-width: 130px">
                            ...
                            >
                                {{ eur(monthReport.expense) }}
                            </div>
                            <DeltaBadge :pct="monthReport.expenseDeltaPct" invert />
                        </div>
                        <div style="flex: 1; min-width: 130px">
                            ...
                            <div
                                ...
                                :style="{ color: monthReport.net >= 0 ? '#20212e' : '#e8544e' }"
                            >
                                {{ eurS(monthReport.net) }}
                            </div>
                            <div style="font-size: 12px; font-weight: 700; color: #0fa3b1; margin-top: 7px">miera úspor {{ monthReport.rate }} %</div>
                        </div>
                        <div
                            style="
                                flex: 1.4;
                            ...
                            "
                        >
                            <div v-if="monthReport.topCategory" style="display: flex; align-items: center; gap: 8px">
                                <span
                                    style="width: 10px; height: 10px; border-radius: 3px; flex-shrink: 0"
                                    :style="{ background: catColor(monthReport.topCategory.category_id) }"
                                ></span>
                                <span style="color: #6a6c7a">Najviac šlo na</span>
                                <span style="font-weight: 700">{{ catName(monthReport.topCategory.category_id) }}</span>
                                <span style="font-weight: 800; margin-left: auto">{{ eur(monthReport.topCategory.amount) }}</span>
                            </div>
                            <div v-if="monthReport.biggestExpense" style="display: flex; align-items: center; gap: 8px">
                                <span
                                    style="width: 10px; height: 10px; border-radius: 3px; flex-shrink: 0"
                                    :style="{ background: catColor(monthReport.biggestExpense.category_id) }"
                                ></span>
                                ...
                                <span style="font-weight: 700; overflow: hidden; text-overflow: ellipsis; white-space: nowrap">{{
                                    monthReport.biggestExpense.note || catName(monthReport.biggestExpense.category_id)
                                }}</span>
                                <span style="font-weight: 800; margin-left: auto">{{ eur(monthReport.biggestExpense.amount) }}</span>
                            </div>
                        </div>
                    </div>
                </Card>
                ...
                <div style="flex: 1.1; min-width: 320px">
                    <Card title="Výdavky podľa kategórie">
                        ...
                        <div style="font-size: 11.5px; color: #b0b2bd; font-weight: 600; margin-top: 14px">
                            Klikni na kategóriu pre detail a vývoj.
                        </div>
                    </Card>
                </div>
