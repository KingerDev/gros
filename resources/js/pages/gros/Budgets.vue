        ...
        <div class="gros-rise">
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(190px, 1fr)); gap: 14px">
                <div style="border-radius: 20px; padding: 20px; color: #fff" :style="{ background: grad, boxShadow: `0 16px 34px ${primarySoft}` }">
                    ...
                    <div class="font-display" style="font-weight: 800; font-size: 28px; letter-spacing: -0.9px; margin-top: 8px">{{ eur(totals.limit) }}</div>
                ...
                <div style="background: #fff; border-radius: 20px; padding: 20px; box-shadow: 0 4px 18px rgba(60, 55, 40, 0.05)">
                    ...
                    <div class="font-display" style="font-weight: 800; font-size: 26px; letter-spacing: -0.8px; margin-top: 8px">{{ eur(totals.spent) }}</div>
                ...
                <div style="background: #fff; border-radius: 20px; padding: 20px; box-shadow: 0 4px 18px rgba(60, 55, 40, 0.05)">
                    ...
                    <div class="font-display" style="font-weight: 800; font-size: 26px; letter-spacing: -0.8px; margin-top: 8px" :style="{ color: left < 0 ? '#e8544e' : '#0fa3b1' }">{{ eurS(left) }}</div>
                    <div style="font-size: 12px; font-weight: 700; color: #9a9cab; margin-top: 6px">{{ totals.overCount === 0 ? 'všetky v norme' : totals.overCount + ' prekročené' }}</div>
                </div>
            </div>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 14px">
                ...
                >
                    <div style="display: flex; align-items: center; gap: 12px">
                        <span style="width: 40px; height: 40px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-weight: 800; font-size: 16px; flex-shrink: 0" :style="{ background: hexToRgba(catColor(b.category_id), 0.14), color: catColor(b.category_id) }">{{ catGlyph(b.category_id) }}</span>
                        <div style="flex: 1; min-width: 0">
                            <div style="font-size: 15px; font-weight: 700">{{ catName(b.category_id) }}</div>
                            <div style="font-size: 12px; color: #9a9cab; font-weight: 600">{{ periodLabels[b.period] }}</div>
                        </div>
                        <span style="font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 20px; white-space: nowrap" :style="{ color: status(b).badgeColor, background: status(b).badgeBg }">{{ status(b).statusLabel }}</span>
                    </div>
                    ...
                    <div style="height: 10px; background: #f1efe8; border-radius: 6px; overflow: hidden; margin-top: 12px">
                        <div :style="{ height: '100%', width: Math.min(100, pct(b)).toFixed(1) + '%', background: status(b).barColor, borderRadius: '6px', transition: 'width .5s ease' }"></div>
                    </div>
