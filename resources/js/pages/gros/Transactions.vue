...
const inSum = computed(() => filtered.value.filter((t) => t.type === 'income').reduce((s, t) => s + Number(t.amount), 0));
const outSum = computed(() => filtered.value.filter((t) => t.type === 'expense').reduce((s, t) => s + Number(t.amount), 0));
...
function filterStyle(v: string) {
    ...
    return { padding: '9px 16px', borderRadius: '12px', fontSize: '13.5px', fontWeight: 700, color: active ? '#fff' : '#61637a', background: active ? primary.value : '#fff', boxShadow: active ? 'none' : '0 2px 8px rgba(60,55,40,.05)' };
}
function periodStyle(v: string) {
    const active = period.value === v;
    return { padding: '7px 13px', borderRadius: '10px', fontSize: '12.5px', fontWeight: 700, color: active ? '#fff' : '#61637a', background: active ? primary.value : '#fff', boxShadow: active ? 'none' : '0 2px 8px rgba(60,55,40,.05)' };
}
    ...
    <GrosLayout title="Transakcie" subtitle="Príjmy a výdavky">
        ...
        <div class="gros-rise">
            <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 14px; flex-wrap: wrap">
                ...
                <button
                    type="button"
                    style="display: flex; align-items: center; gap: 7px; background: #fff; color: #20212e; font-weight: 700; font-size: 13px; padding: 9px 14px; border-radius: 12px; box-shadow: 0 2px 8px rgba(60, 55, 40, 0.06); white-space: nowrap"
                    @click="exportCsv"
                >
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12M8 11l4 4 4-4" /><path d="M4 17v2a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-2" /></svg>
                    Export CSV
                </button>
            </div>
            ...
            <div style="display: flex; align-items: center; justify-content: space-between; gap: 12px; margin: 0 4px 12px; flex-wrap: wrap">
                <div style="font-size: 13px; font-weight: 600; color: #6a6c7a">{{ filtered.length }} transakcií · {{ periodLabel }}</div>
            ...
            <div style="background: #fff; border-radius: 20px; padding: 8px; box-shadow: 0 4px 18px rgba(60, 55, 40, 0.05)">
                ...
                >
                    ...
                    <template v-else>
                        ...
                        <div style="flex: 1; min-width: 0">
                            ...
                            <div style="font-size: 12px; color: #9a9cab; font-weight: 500">{{ catName(t.category_id) }} · {{ t.account?.name ?? '—' }} · {{ formatDate(t.date) }}</div>
                        </div>
                    ...
                    <button
                        type="button"
                        style="width: 32px; height: 32px; border-radius: 10px; background: transparent; color: #c4c2ba; display: flex; align-items: center; justify-content: center; flex-shrink: 0"
                        title="Zmazať"
                        @click.stop="del(t)"
                    >
                ...
                <div v-if="!filtered.length" style="padding: 40px 16px; text-align: center; color: #b0b2bd; font-weight: 600; font-size: 14px">Žiadne transakcie v tomto období</div>
            </div>
        </div>
